...
void outputDataInFile(int* sizeData, float* timeData, int size)
{
    ...
    for (int i = 0; i < size; i++)
        fprintf(file, "%d ", sizeData[i]);

    fprintf(file, "\ntime (sec):\n");

    for (int i = 0; i < size; i++)
        fprintf(file, "%f ", timeData[i]);
}

int main()
{
    int startSize = 3, maxSize = 4053, step = 50;
    int dataCount = (maxSize - startSize) / step + 1;

    int* sizeData = (int*)malloc(dataCount * sizeof(int));
    float* timeData = (float*)malloc(dataCount * sizeof(float));
    ...
    outputDataInFile(sizeData, timeData, dataCount);
