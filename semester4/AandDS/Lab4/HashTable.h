...
namespace Lab4
{
	...
	inline bool HashTable<K, V>::ChainsOfCollisions::Remove(K key)
	{
		...
		if (prev == nullptr && next == nullptr) //перва€ и единственна€ в цепочке коллизий
		{
			cellToRemove->state = Cell::State::Deleted;
		}
		else if (prev == nullptr) //перва€, но не единственна€ в цепочке коллизий
		{
		...
		else if (next == nullptr) //последн€€, но не единственна€ в цепочке коллизий
		{
			prev->next = nullptr;
			delete cellToRemove;
		}
		else //не перва€, не последн€€, не единственна€ в цепочке коллизий
		{
			prev->next = next;
			delete cellToRemove;
		}
